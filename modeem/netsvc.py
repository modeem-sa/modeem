...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('modeem.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    if sys.version_info[:2] == (3, 9):
        ...
        warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='modeem')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='modeem.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'modeem.http.rpc.request:INFO',
    'modeem.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['modeem:DEBUG', 'modeem.sql_db:INFO', 'modeem.http.rpc:DEBUG'],
    'debug_rpc': ['modeem:DEBUG', 'modeem.sql_db:INFO', 'modeem.http.rpc.request:DEBUG'],
    'debug': ['modeem:DEBUG', 'modeem.sql_db:INFO'],
    'debug_sql': ['modeem.sql_db:DEBUG'],
    'info': [],
    'runbot': ['modeem:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['modeem:WARNING', 'werkzeug:WARNING'],
    'error': ['modeem:ERROR', 'werkzeug:ERROR'],
    'critical': ['modeem:CRITICAL', 'werkzeug:CRITICAL'],
}
