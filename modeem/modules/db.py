...
def is_initialized(cr):
    ...
    """
    return modeem.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = modeem.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with modeem.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in modeem.modules.get_modules():
        mod_path = modeem.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = modeem.modules.get_manifest(i)
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by Modeem.
