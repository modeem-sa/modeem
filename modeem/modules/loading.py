...
import modeem
import modeem.modules.db
import modeem.modules.graph
import modeem.modules.migration
import modeem.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('modeem.tests')
...
def force_demo(cr):
    ...
    """
    graph = modeem.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    processed_modules = []
    loaded_modules = []
    registry = modeem.registry(cr.dbname)
    migrations = modeem.modules.migration.MigrationManager(cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = modeem.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = modeem.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['modeem.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = modeem.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                ...
                tests_t0, tests_q0 = time.time(), modeem.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = modeem.sql_db.sql_counter - tests_q0
        ...
        extra_queries = modeem.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   modeem.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not modeem.modules.db.is_initialized(cr):
        ...
        if update_module and modeem.tools.table_exists(cr, 'ir_model_fields'):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['modeem.addons.%s' % (pkg.name,)]
                ...
                registry = modeem.modules.registry.Registry.new(
