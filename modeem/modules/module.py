...
import modeem
import modeem.tools as tools
import modeem.release as release
...
# ad_paths is a deprecated alias, please use modeem.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"modeem.modules.module.ad_paths" is a deprecated proxy to '
        '"modeem.addons.__path__".', DeprecationWarning, stacklevel=2)
    return modeem.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "modeem.addons".',
    ...
    def load_module(self, name):
        ...
        modeem_name = re.sub(r'^openerp.addons.(\w+)$', r'modeem.addons.\g<1>', name)

        modeem_module = sys.modules.get(modeem_name)
        if not modeem_module:
            modeem_module = importlib.import_module(modeem_name)

        sys.modules[name] = modeem_module

        return modeem_module

class ModeemHook(object):
    """ Makes modeem package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to modeem.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'modeem\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `modeem.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^modeem.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("modeem.addons.base.maintenance.migrations", "modeem.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``modeem.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook modeem.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in modeem.addons.__path__:
        modeem.addons.__path__.append(dd)

    # hook modeem.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in modeem.addons.__path__:
            modeem.addons.__path__.append(ad)

    # hook modeem.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in modeem.addons.__path__ and os.path.isdir(base_path):
        modeem.addons.__path__.append(base_path)

    # hook modeem.upgrade on upgrade-path
    from modeem import upgrade
    ...
    # create decrecated module alias from modeem.addons.base.maintenance.migrations to modeem.upgrade
    spec = importlib.machinery.ModuleSpec("modeem.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["modeem.addons.base.maintenance"] = maintenance_pkg
    sys.modules["modeem.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to modeem and "crm"-like to modeem.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, ModeemHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in modeem.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = modeem.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in modeem.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('modeem.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['modeem.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in modeem.addons.__path__:
