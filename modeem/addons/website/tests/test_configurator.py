...
import modeem.tests
from modeem.addons.iap.tools.iap_tools import iap_jsonrpc_mocked
from modeem.tools import mute_logger

class TestConfiguratorCommon(modeem.tests.HttpCase):
    ...
    def setUp(self):
        ...
        iap_patch = patch('modeem.addons.iap.tools.iap_tools.iap_jsonrpc', iap_jsonrpc_mocked_configurator)
        self.startPatcher(iap_patch)

        patcher = patch('modeem.addons.website.models.ir_module_module.IrModuleModule._theme_upgrade_upstream', wraps=self._theme_upgrade_upstream)
