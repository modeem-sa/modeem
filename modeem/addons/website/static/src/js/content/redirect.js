 ...
 * corner to provide the authenticated user an access to his modeem backend.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!window.frameElement) {
        ...
        // - Having an event that's fire only when the page is from Modeem avoids
        //   weird behaviours. (e.g. if we want to clear out the iframe, it might
        //   fire an DOMContentLoaded on a non modeem page)
        window.frameElement.dispatchEvent(new CustomEvent('ModeemFrameContentLoaded'));
    }
});
