...
export class WebsitePreview extends Component {
    setup() {
        ...
        onMounted(() => {
            ...
            // ModeemFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('ModeemFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        ...
        /**
         * This removes the 'Modeem' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('ModeemFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('ModeemFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('ModeemFrameContentLoaded', resolve, { once: true });
            if (url) {
