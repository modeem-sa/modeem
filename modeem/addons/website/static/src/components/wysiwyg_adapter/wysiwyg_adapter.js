...
export class WysiwygAdapterComponent extends ComponentAdapter {
    ...
    setup() {
        ...
        useEffect(() => {
            const initWysiwyg = async () => {
                // Disable ModeemEditor observer's while setting up classes
                this.widget.modeemEditor.observerUnactive();
                ...
                // the dirty observer (not to be confused with modeemEditor
                // observer) as the widgets might trigger DOM mutations.
                this._setObserver();
                this.widget.modeemEditor.observerActive();
            };
    ...
    leaveEditMode({ onLeave, forceLeave, onStay, reloadIframe = true } = {}) {
        const leave = () => {
            ...
            // The wysiwyg is destroyed to avoid listeners from the ModeemEditor
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.widget.modeemEditor.filterMutationRecords(records);
            ...
            this.widget.modeemEditor.automaticStepSkipStack();
        ...
        this.widget.modeemEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.widget.modeemEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
        ...
        const commands = [
            {
                ...
                fontawesome: 'fa-info',
                isDisabled: () => !this.widget.modeemEditor.isSelectionInBlockRoot(),
                callback: () => {
