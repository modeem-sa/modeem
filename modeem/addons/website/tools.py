...
import modeem
from modeem.tests.common import HttpCase, HOST
from modeem.tools.misc import DotDict, frozendict
...
def MockRequest(
    ...
    request = Mock(
        ...
        type='http',
        future_response=modeem.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            modeem.http.get_default_session(),
    ...
    #
    #   router = modeem.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        modeem.http._request_stack.push(request)
        s.callback(modeem.http._request_stack.pop)
        s.enter_context(patch('modeem.http.root.get_db_router', router))
