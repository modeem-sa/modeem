...
export function usePartnerAutocomplete() {
    const keepLastModeem = new KeepLast();
    ...
    function autocomplete(value, isVAT = false) {
        value = value.trim();
        let modeemSuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const modeemPromise = getModeemSuggestions(value, isVAT).then((suggestions) => {
                modeemSuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with Modeem result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = modeemSuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            modeemSuggestions.push(suggestion);
                        }
                    });
                }

                modeemSuggestions = modeemSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                modeemSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(modeemSuggestions);
            };

            whenAll([modeemPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use Modeem Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getModeemSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastModeem.add(prom);
