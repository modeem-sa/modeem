...
from modeem import api, fields, models, registry, _
from modeem.tools import ormcache_context, email_normalize
from modeem.exceptions import UserError
from modeem.osv import expression

from modeem.addons.google_calendar.utils.google_event import GoogleEvent
from modeem.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from modeem.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Modeem database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Modeem.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Modeem DB (_sync_google2modeem).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_modeem2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2modeem(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in Modeem. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Modeem
        :return: synchronized modeem recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        modeem_values = [
            dict(self._modeem_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_modeem = self.with_context(dont_notify=True)._create_from_google(new, modeem_values)
        cancelled = existing.cancelled()
        cancelled_modeem = self.browse(cancelled.modeem_ids(self.env))
        cancelled_modeem._cancel()
        synced_records = new_modeem + cancelled_modeem
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and modeem server time are different
            updated = parse(gevent.updated)
            modeem_record = self.browse(gevent.modeem_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not modeem_record.write_date or updated >= pytz.utc.localize(modeem_record.write_date):
                vals = dict(self._modeem_values(gevent, default_reminders), need_sync=False)
                modeem_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= modeem_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Modeem to Google
    ...
    @api.model
    def _modeem_values(self, google_event: GoogleEvent, default_reminders=()):
