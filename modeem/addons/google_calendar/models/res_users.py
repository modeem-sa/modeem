...
from modeem import api, fields, models, Command
from modeem.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from modeem.addons.google_calendar.models.google_sync import google_calendar_token
from modeem.loglevels import exception_to_unicode
...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2modeem(recurrences)
        synced_events = self.env['calendar.event']._sync_google2modeem(events - recurrences, default_reminders=default_reminders)

        # Modeem -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_modeem2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_modeem2google(calendar_service)
