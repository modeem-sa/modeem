...
from modeem.addons.google_account.models.google_service import GoogleService
from modeem.addons.google_calendar.models.res_users import User
...
@tagged('modeem2google')
@patch.object(User, '_get_google_calendar_token', lambda user: 'dummy-token')
class TestSyncModeem2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_modeem2google(self.google_service)
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=616):
            ...
            events._sync_modeem2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'modeembot@example.com', 'self': True},
            'attendees': [{'email': 'modeembot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_modeem_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_modeem2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_modeem_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [{'email': 'modeembot@example.com', 'responseStatus': 'accepted'}],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_modeem_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
    ...
    def test_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=True)._sync_modeem2google(self.google_service)
    ...
    def test_not_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=False)._sync_modeem2google(self.google_service)
        self.call_post_commit_hooks()
