...
from modeem.tests.common import new_test_user
from modeem.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from modeem.addons.google_calendar.utils.google_calendar import GoogleEvent
from modeem.tools import html2plaintext
from modeem import Command

class TestSyncGoogle2Modeem(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2modeem(google_recurrence)
        self.env['calendar.event']._sync_google2modeem(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'modeemcalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2modeem(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2modeem(gevent)
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'modeembot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_modeem_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2modeem(GoogleEvent([values]))
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "modeembot@example.com", "state": "accepted",
                },
            ],
            'updated': self.now,
        }
    ...
    def test_recurrence_no_duplicate(self):
        ...
        self.env['calendar.recurrence']._sync_google2modeem(google_events)
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2modeem(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_attendees_same_event_both_share(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'c.c@example.com', 'responseStatus': 'needsAction'},
                          {'email': 'modeembot@example.com', 'responseStatus': 'accepted'},],
            'extendedProperties': {'shared': {'%s_modeem_id' % self.env.cr.dbname: event.id,
    ...
    def test_several_attendee_have_the_same_mail(self):
        """
        In google, One mail = One attendee but on Modeem, some partners could share the same mail
        ...
        self.assertEqual(mails, ['dalton@example.com', 'modeembot@example.com'])
        gevent = GoogleEvent([{
            'id': google_id,
            'description': 'coucou',
            "updated": self.now,
            'organizer': {'email': 'modeembot@example.com', 'self': True},
            'summary': False,
            'visibility': 'public',
            'attendees': [],
            'reminders': {'useDefault': True},
        ...
        # User attendee removed but gevent owner might be added after synch.
        mails = sorted(set(event.attendee_ids.mapped('email')))
        self.assertEqual(mails, ['modeembot@example.com'])
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        event = self.env['calendar.event']._sync_google2modeem(GoogleEvent([values]))
    ...
    def test_event_with_meeting_url(self):
        values = {
            ...
            'conferenceData': {
                'entryPoints': [{
                    ...
                    'uri': 'https://meet.google.com/modeem-random-test',
                    'label': 'meet.google.com/modeem-random-test'
                }, {
                    'entryPointType': 'more',
                    'uri':'https://tel.meet/modeem-random-test?pin=42424242424242',
                    'pin':'42424242424242'
                }]
            }
        }
        ...
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(event.videocall_location, 'https://meet.google.com/modeem-random-test')
        self.assertGoogleAPINotCalled()
