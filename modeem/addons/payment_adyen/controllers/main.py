...
from modeem.addons.payment_adyen import utils as adyen_utils
from modeem.addons.payment_adyen.const import CURRENCY_DECIMALS
...
class AdyenController(http.Controller):
    ...
    def adyen_return_from_3ds_auth(self, **data):
        ...
        The route is flagged with `save_session=False` to prevent Modeem from assigning a new session
        to the user if they are redirected to this route with a POST request. Indeed, as the session
        cookie is created without a `SameSite` attribute, some browsers that don't implement the
        recommended default `SameSite=Lax` behavior will not include the cookie in the redirection
        request from the payment provider to Modeem. As the redirection to the '/payment/status' page
        ...
        # of Modeem in some cases. For instance, when a 3DS1 authentication is required, or for
