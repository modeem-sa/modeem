...
from modeem.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Modeem event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Modeem events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Modeem events
        # -----------------------------------------------------------------------------------------

        self.expected_modeem_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_modeem_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_modeem_recurrency_events_from_outlook = [
            {
    ...
    def assert_modeem_event(self, modeem_event, expected_values):
        """
        Assert that an Modeem event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        modeem_event_values = modeem_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(modeem_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, modeem_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_modeem_recurrence(self, modeem_recurrence, expected_values):
        """
        Assert that an Modeem recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        modeem_recurrence_values = modeem_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, modeem_recurrence_values.get(k), msg=f"'{k}' mismatch")
