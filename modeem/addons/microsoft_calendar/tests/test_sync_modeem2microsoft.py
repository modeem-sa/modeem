...
class TestSyncModeem2Microsoft(TransactionCase):
    ...
    def test_restart_synchronization(self):
        ...
        event.with_user(user)._sync_modeem2microsoft(self.microsoft_service)
        ...
        self.assertMicrosoftEventPatched(event.microsoft_id, {
            ...
            'singleValueExtendedProperties': [{
                    'id': 'String {%s} Name modeem_id' % microsoft_guid,
                    'value': str(event.id),
                }, {
                    'id': 'String {%s} Name owner_modeem_id' % microsoft_guid,
                    'value': str(user.id),
                }
            ]
        })
