...
import logging
import requests
from modeem.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from modeem.addons.microsoft_account.models.microsoft_service import DEFAULT_MICROSOFT_TOKEN_ENDPOINT
from modeem.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Modeem
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2modeem(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Modeem -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_modeem2microsoft()
        ...
        (events - synced_events)._sync_modeem2microsoft()
