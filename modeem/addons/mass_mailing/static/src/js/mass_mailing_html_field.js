...
import { getRangePosition } from '@web_editor/js/editor/modeem-editor/src/utils/utils';
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges() {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.modeemEditor.historyPauseSteps();
            ...
            this.wysiwyg.modeemEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.modeemEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.modeemEditor.historyUnpauseSteps();
            this.wysiwyg.modeemEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.modeem-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.modeemEditor.observerFlush();
        this.wysiwyg.modeemEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $snippetsSideBar.on('click', '.o_codeview_btn', () => {
            this.wysiwyg.modeemEditor.observerUnactive();
            $codeview.toggleClass('d-none');
            this.wysiwyg.getEditable().toggleClass('d-none');
            this.wysiwyg.modeemEditor.observerActive();
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Modeem editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.modeemEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.modeemEditor.editable.querySelector('p');
                if (p) {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.modeemEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.modeemEditor.historyStep(true);
