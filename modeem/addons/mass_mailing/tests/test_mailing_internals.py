...
from modeem.addons.mass_mailing.tests.common import MassMailCommon
from modeem.exceptions import ValidationError
from modeem.sql_db import Cursor
from modeem.tests.common import users, Form
...
class TestMassMailValues(MassMailCommon):
    ...
    def test_mailing_computed_fields_domain_w_filter(self):
        ...
        filter_1, filter_2, filter_3 = self.env['mailing.filter'].create([
            ...
            {'name': 'Email based',
             'mailing_domain' : [('email', 'ilike', 'info@modeem.com')],
             'mailing_model_id': self.env['ir.model']._get('res.partner').id,
            }
        ])
        ...
        mailing.mailing_domain = "[('email', 'ilike', 'info_be@modeem.com')]"
...
class TestMassMailFeatures(MassMailCommon, CronMixinCase):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
<t t-set="url" t-value="'www.modeem.com'"/>
<t t-set="httpurl" t-value="'https://www.modeem.eu'"/>
Website0: <a id="url0" t-attf-href="https://www.modeem.tz/my/{{object.name}}">https://www.modeem.tz/my/<t t-esc="object.name"/></a>
Website1: <a id="url1" href="https://www.modeem.be">https://www.modeem.be</a>
...
Email: <a id="url5" href="mailto:test@modeem.com">test@modeem.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'new',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.modeem.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.modeem.be', True),
                              ('url2', 'https://www.modeem.com', True),
                              ('url3', 'https://www.modeem.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@modeem.com', False)]:
