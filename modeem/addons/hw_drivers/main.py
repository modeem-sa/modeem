...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Modeem database
        """
        server = helpers.get_modeem_server_url()
        if server:
            subject = helpers.read_file_first_line('modeem-subject.conf')
            if subject:
            ...
            _logger.warning('Modeem server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the modeem server with the updates
        """
