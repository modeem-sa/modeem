...
from modeem import _, http, service
from modeem.tools.func import lazy_property
...
class IoTRestart(Thread):
    """
    Thread to restart modeem server in IoT Box when we must return a answer before
    """
    ...
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'ModeemTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Modeem DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/modeem/', '--git-dir=/home/pi/modeem/.git']
                ...
                if db_branch != local_branch:
                    with writable():
                        subprocess.check_call(["rm", "-rf", "/home/pi/modeem/addons/hw_drivers/iot_handlers/drivers/*"])
                        subprocess.check_call(["rm", "-rf", "/home/pi/modeem/addons/hw_drivers/iot_handlers/interfaces/*"])
...
def check_image():
    ...
    """
    url = 'https://nightly.modeem.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def save_conf_server(url, token, db_uuid, enterprise_code):
    ...
    """
    write_file('modeem-remote-server.conf', url)
    write_file('token', token)
    write_file('modeem-db-uuid.conf', db_uuid or '')
    write_file('modeem-enterprise-code.conf', enterprise_code or '')
...
def get_modeem_server_url():
    if platform.system() == 'Linux':
        ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
        if not ap:
            return False
    return read_file_first_line('modeem-remote-server.conf')
...
def get_version():
    if platform.system() == 'Linux':
        return read_file_first_line('/var/modeem/iotbox_version')
...
def load_certificate():
    """
    Send a request to Modeem with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('modeem-db-uuid.conf')
    enterprise_code = read_file_first_line('modeem-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.modeem.com/modeem-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('modeem-subject.conf', result['subject_cn'])
            if platform.system() == 'Linux':
                with writable():
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Modeem server
    """
    server = get_modeem_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                with writable():
                    drivers_path = ['modeem', 'addons', 'hw_drivers', 'iot_handlers']
...
def load_iot_handlers():
    """
    This method loads local files: 'modeem/addons/hw_drivers/iot_handlers/drivers' and
    'modeem/addons/hw_drivers/iot_handlers/interfaces'
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Modeem\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Modeem\\downloaded_file.zip', 'C:\\Program Files\\Modeem\\new_folder'))
