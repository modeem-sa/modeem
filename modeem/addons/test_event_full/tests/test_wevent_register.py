...
from modeem.addons.test_event_full.tests.common import TestWEventCommon
...
class TestWEventRegister(TestWEventCommon):
    ...
    def test_register(self):
        with freeze_time(self.reference_now, tick=True):
            self.browser_js(
                '/event',
                'modeem.__DEBUG__.services["web_tour.tour"].run("wevent_register")',
                'modeem.__DEBUG__.services["web_tour.tour"].tours.wevent_register.ready',
                login=None
            )
