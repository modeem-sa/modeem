...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test("Domain of simple date filter", async function (assert) {
        ...
        const chartId = model.getters.getModeemChartIds()[0];
        await addGlobalFilter(
            model,
            {
                filter: {
    ...
    QUnit.test("field matching is removed when an Modeem chart is deleted", async function (assert) {
        ...
        assert.deepEqual(model.getters.getModeemChartFieldMatching(chartId, filter.id), matching);
        model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        assert.throws(
            () => model.getters.getModeemChartFieldMatching(chartId, filter.id),
