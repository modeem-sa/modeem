...
QUnit.module(
    ...
    () => {
        QUnit.test(
            "icon external link isn't on the chart when its not linked to an modeem menu",
            async function (assert) {
                ...
                await nextTick();
                const modeemMenu = model.getters.getChartModeemMenu(chartId);
                assert.equal(modeemMenu, undefined, "No menu linked with the chart");
        ...
        QUnit.test(
            "icon external link is on the chart when its linked to an modeem menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                ...
                const chartMenu = model.getters.getChartModeemMenu(chartId);
                assert.equal(chartMenu.id, 1, "Modeem menu is linked to chart");
                await nextTick();
                const externalRefIcon = fixture.querySelector(".o-chart-external-link");
        ...
        QUnit.test(
            "icon external link is not on the chart when its linked to a wrong modeem menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                ...
                model.dispatch("LINK_ODOO_MENU_TO_CHART", {
                    chartId,
                    modeemMenuId: "menu which does not exist",
                });
                const chartMenu = model.getters.getChartModeemMenu(chartId);
        ...
        QUnit.test(
            "click on icon external link on chart redirect to the modeem menu",
            async function (assert) {
                ...
                const chartMenu = model.getters.getChartModeemMenu(chartId);
                assert.equal(chartMenu.id, 2, "Modeem menu is linked to chart");
                await nextTick();
        ...
        QUnit.test(
            "Click on chart in dashboard mode redirect to the modeem menu",
            async function (assert) {
                const doActionStep = "doAction";
                mockActionService(assert, doActionStep);
                const model = await createModelWithDataSource({
                ...
                assert.verifySteps(
                    [doActionStep],
                    "Clicking on a chart while on dashboard mode redirect to the modeem menu"
                );
            }
        );
        ...
        QUnit.test("can use menus xmlIds instead of menu ids", async function (assert) {
            ...
            model.dispatch("LINK_ODOO_MENU_TO_CHART", {
                chartId,
                modeemMenuId: "documents_spreadsheet.test.menu2",
            });
            await nextTick();
