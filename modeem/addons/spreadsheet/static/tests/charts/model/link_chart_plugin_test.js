    ...
    () => {
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    exportedData.chartModeemMenusReferences[chartId],
                    1,
                    "Link to modeem menu is exported"
                );
                const importedModel = new Model(exportedData, { evalContext: { env } });
                const chartMenu = importedModel.getters.getChartModeemMenu(chartId);
                assert.equal(chartMenu.id, 1, "Link to modeem menu is imported");
            }
        );
        ...
        QUnit.test("Can undo-redo a LINK_ODOO_MENU_TO_CHART", async function (assert) {
            ...
            assert.equal(model.getters.getChartModeemMenu(chartId).id, 1);
            model.dispatch("REQUEST_UNDO");
            assert.equal(model.getters.getChartModeemMenu(chartId), undefined);
