...
QUnit.test("Modeem formulas are migrated", (assert) => {
    const data = {
        sheets: [
            {
                cells: {
...
QUnit.test("Pivot 'day' arguments are migrated", (assert) => {
        ...
        sheets: [
            {
                cells: {
                    ...
                    A3: { content: `=modeem.pivot("1","21/07/2021")` },
                    A4: { content: `=ODOO.PIVOT("1","test")` },
                    A5: { content: `=modeem.pivot("1","21/07/2021")+"21/07/2021"` },
    ...
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=modeem.pivot("1","07/21/2021")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=ODOO.PIVOT("1","test")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A5.content,
        `=modeem.pivot("1","07/21/2021")+"21/07/2021"`
    );
...
QUnit.test("Modeem version is exported", (assert) => {
    const model = new Model();
    assert.strictEqual(model.exportData().modeemVersion, ODOO_VERSION);
});
