...
export function migrate(data) {
    let _data = load(data, !!modeem.debug);
    const version = _data.modeemVersion || 0;
    if (version < 1) {
...
export default class ModeemVersion extends CorePlugin {
    export(data) {
        data.modeemVersion = ODOO_VERSION;
    }
}

ModeemVersion.getters = [];

corePluginRegistry.add("modeemMigration", ModeemVersion);
