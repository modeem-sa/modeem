...
xml_id_url_prefix = "modeem://ir_menu_xml_id/"

modeem_view_link_prefix = "modeem://view/"


def modeem_charts(data):
    """return all modeem chart definition in the spreadsheet"""
    figures = []
    for sheet in data["sheets"]:
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet["figures"]
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("modeem_")
        ]
    return figures
...
def modeem_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like modeem://view/{... view data...}
    """
    return [
        json.loads(url[len(modeem_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(modeem_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = modeem_charts(data)
    modeem_version = data.get("modeemVersion", 1)
    if modeem_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, modeem_charts(data)).items(),
        extract_fields(modeem_view_fields, modeem_view_links(data)).items(),
...
def xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartModeemMenusReferences", {}).values()) | {
