...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if modeembot is in a chat with a user or
        if someone pinged modeembot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/modeembot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == modeembot_id or values.get("message_type") != "comment" and not command:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=modeembot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if modeembot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.modeembot_state = "onboarding_command"
                self.env.user.modeembot_failed = False
                return _("Great! 👍<br/>To access special commands, <b>start your sentence with</b> <span class=\"o_modeembot_command\">/</span>. Try getting help.")
            elif modeembot_state == 'onboarding_command' and command == 'help':
                self.env.user.modeembot_state = "onboarding_ping"
                self.env.user.modeembot_failed = False
                return _("Wow you are a natural!<br/>Ping someone with @username to grab their attention. <b>Try to ping me using</b> <span class=\"o_modeembot_command\">@ModeemBot</span> in a sentence.")
            elif modeembot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.modeembot_state = "onboarding_attachement"
                self.env.user.modeembot_failed = False
                return _("Yep, I am here! 🎉 <br/>Now, try <b>sending an attachment</b>, like a picture of your cute dog...")
            elif modeembot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.modeembot_state = "idle"
                self.env.user.modeembot_failed = False
                return _("I am a simple bot, but if that's a dog, he is the cutest 😊 <br/>Congratulations, you finished this tour. You can now <b>close this chat window</b>. Enjoy discovering Modeem.")
            elif modeembot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.modeembot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif modeembot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or modeembot_state == 'idle':
                return _("Unfortunately, I'm just a bot 😞 I don't understand! If you need help discovering our product, please check "
                         "<a href=\"https://www.modeem.com/documentation\" target=\"_blank\">our documentation</a> or "
                         "<a href=\"https://www.modeem.com/slides\" target=\"_blank\">our videos</a>.")
            else:
                # repeat question
                if modeembot_state == 'onboarding_emoji':
                    self.env.user.modeembot_failed = True
                    return _("Not exactly. To continue the tour, send an emoji: <b>type</b> <span class=\"o_modeembot_command\">:)</span> and press enter.")
                elif modeembot_state == 'onboarding_attachement':
                    self.env.user.modeembot_failed = True
                    return _("To <b>send an attachment</b>, click on the <i class=\"fa fa-paperclip\" aria-hidden=\"true\"></i> icon and select a file.")
                elif modeembot_state == 'onboarding_command':
                    self.env.user.modeembot_failed = True
                    return _("Not sure what you are doing. Please, type <span class=\"o_modeembot_command\">/</span> and wait for the propositions. Select <span class=\"o_modeembot_command\">help</span> and press enter")
                elif modeembot_state == 'onboarding_ping':
                    self.env.user.modeembot_failed = True
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write <span class=\"o_modeembot_command\">@ModeemBot</span> and select me.")
                return random.choice([
                    _("I'm not smart enough to answer your question.<br/>To follow my guide, ask: <span class=\"o_modeembot_command\">start the tour</span>."),
                    _("Hmmm..."),
                    _("I'm afraid I don't understand. Sorry!"),
                    _("Sorry I'm sleepy. Or not! Maybe I'm just trying to hide my unawareness of human language...<br/>I can show you features if you write: <span class=\"o_modeembot_command\">start the tour</span>.")
                ])
        return False
    ...
    def _is_bot_pinged(self, values):
        modeembot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        return modeembot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        modeembot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return modeembot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.modeembot_failed
