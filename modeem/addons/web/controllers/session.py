...
from modeem.modules import module
from modeem.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Modeem Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if pre_uid != request.session.uid:
            # Crapy workaround for unupdatable Modeem Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = modeem.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = modeem.api.Environment(cr, request.session.uid, request.session.context)
    ...
    def account(self):
        ...
        return 'https://accounts.modeem.com/oauth2/auth?' + url_encode(params)
