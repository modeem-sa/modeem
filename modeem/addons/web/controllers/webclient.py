...
from modeem.modules import get_manifest, get_resource_path
...
@lazy
def CONTENT_MAXAGE():
    warnings.warn("CONTENT_MAXAGE is a deprecated alias to modeem.http.STATIC_CACHE_LONG", DeprecationWarning)
...
class WebClient(http.Controller):
    ...
    def bootstrap_translations(self, mods=None):
        ...
        if mods is None:
            mods = modeem.conf.server_wide_modules or []
            if request.db:
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (modeem.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return modeem.service.common.exp_version()
