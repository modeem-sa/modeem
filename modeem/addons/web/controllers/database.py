...
from modeem.http import content_disposition, dispatch_rpc, request, Response
from modeem.service import db
from modeem.tools.misc import file_open, str2bool
from modeem.tools.translate import _

from modeem.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = modeem.tools.config.verify_admin_password('admin')
        d['list_db'] = modeem.tools.config['list_db']
        d['langs'] = modeem.service.db.exp_list_lang()
        d['countries'] = modeem.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = modeem.service.db.list_db_incompatible(d['databases'])
        except modeem.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = modeem.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            modeem.service.db.check_super(master_pwd)
            ...
            dump_stream = modeem.service.db.dump_db(name, None, backup_format)
