...
try:
    ...
    from modeem.tools._vendor.send_file import send_file

import modeem
import modeem.modules.registry
from modeem import http, _
from modeem.exceptions import AccessError, UserError
from modeem.http import request, Response
from modeem.modules import get_resource_path
from modeem.tools import file_open, file_path, replace_exceptions
from modeem.tools.mimetypes import guess_mimetype
from modeem.tools.image import image_guess_size_from_field_name


_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Modeem is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Modeem is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if modeem.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=modeem.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        dbname = request.db
        uid = (request.session.uid if dbname else None) or modeem.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                registry = modeem.modules.registry.Registry(dbname)
