...
from modeem.http import content_disposition, request
from modeem.tools import lazy_property, osutil, pycompat
from modeem.tools.misc import xlsxwriter
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name='',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: modeem.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Modeem's export data to whatever the
