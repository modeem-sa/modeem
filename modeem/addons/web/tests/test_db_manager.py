...
import modeem
from modeem.tests.common import BaseCase, HttpCase, tagged
...
class TestDatabaseOperations(BaseCase):
    def setUp(self):
        ...
        self.verify_admin_password_patcher = patch(
            'modeem.tools.config.verify_admin_password', self.password.__eq__,
        )
    ...
    def list_dbs_filtered(self):
        return set(db for db in modeem.service.db.list_dbs(True) if re.match(config['dbfilter'], db))
    ...
    def test_database_creation(self):
        ...
        self.assertTrue(modeem.tools.config.verify_admin_password(self.password))
