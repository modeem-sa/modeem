...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_modeem.menu_adyen_account",
        "payment_modeem.root_adyen_menu",
    ];

    const { isEnterprise } = modeem.info;
    const { onWillStart } = owl;
    let appsMenusOnly = false;
    const isStudioInstalled = "@web_studio/studio_service" in modeem.__DEBUG__.services;
    let actionCount = 0;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = modeem.__WOWL_DEBUG__.root.env;
        env.bus.addEventListener("ACTION_MANAGER:UI-UPDATED", () => {
            actionCount++;
        });

        const AbstractController = modeem.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = modeem.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = modeem.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting Modeem module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = modeem.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`Modeem flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
