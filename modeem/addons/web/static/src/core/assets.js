 ...
 */
export const templates = new DOMParser().parseFromString("<modeem/>", "text/xml");
...
export const _loadBundle = (assets.loadBundle = async function loadBundle(desc) {
    ...
    for (const urlData of desc.jsLibs || []) {
        if (typeof urlData === "string") {
            ...
            if (bundle) {
                await modeem.ready(bundle[1] + ".bundle.xml");
            }
        } else {
            // parallel loading
...
import { Component, xml, onWillStart } from "@modeem/owl";
/**
