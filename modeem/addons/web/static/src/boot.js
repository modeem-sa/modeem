...
(function () {
    ...
    if (!globalThis.modeem) {
        globalThis.modeem = {};
    }
    var modeem = globalThis.modeem;
    var debug = modeem.debug;
    ...
    modeem.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__modeem_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: modeem.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = modeem.__DEBUG__.getDependencies(job.name);
            ...
            missing = modeem.__DEBUG__.getMissingJobs();
            failed = modeem.__DEBUG__.getFailedJobs();
     ...
     */
    modeem.ready = async function (serviceName) {
        function match(name) {
    ...
    modeem.runtimeImport = function (moduleName) {
