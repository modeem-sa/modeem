...
import { Component, xml } from "@modeem/owl";
...
QUnit.test("checks that href is correctly used", async (assert) => {
    ...
    assert.strictEqual(command.querySelector("a").getAttribute("href"), "https://www.modeem.com");
    // Check that we get url when doing ctrl+enter on a command having a link inside it
    triggerHotkey("control+enter");
    await nextTick();
    assert.verifySteps(["https://www.modeem.com"]);
