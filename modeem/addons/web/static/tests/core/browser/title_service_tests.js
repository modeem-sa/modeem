...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Modeem" });
    assert.strictEqual(env.services.title.current, "Modeem");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Modeem", chat: null });
    assert.strictEqual(env.services.title.current, "Modeem");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Modeem - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Modeem", action: "Import" });
    assert.strictEqual(env.services.title.current, "Modeem - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Modeem", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Modeem - Import"); // parts is a copy!
});
