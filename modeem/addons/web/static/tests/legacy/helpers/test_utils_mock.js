modeem.define('web.test_utils_mock', function (require) {
"use strict";
...
const { Component } = require("@modeem/owl");
...
function _mockGlobalObjects(params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            modeem.debug = params.config.debug;
        }
    }
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Modeem widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in ModeemAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by modeem
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Modeem
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other Modeem events bubbling up are intercepted
