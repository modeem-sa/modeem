...
import { Component, xml, EventBus } from "@modeem/owl";
...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test("proper stringification in debug mode tooltip", async function (assert) {
        patchWithCleanup(modeem, { debug: true });
    ...
    QUnit.test("no 'oh snap' error in form view in dialog", async (assert) => {
        ...
        registry.category("services").add("error", errorService);
        registry.category("error_dialogs").add("modeem.exceptions.UserError", WarningDialog);
        // remove the override in qunit.js that swallows unhandledrejection errors
        // s.t. we let the error service handle them
        const originalOnUnhandledRejection = window.onunhandledrejection;
        window.onunhandledrejection = () => {};
        ...
        const webClient = await createWebClient({
            ...
            mockRPC(route, { method }) {
                if (method === "create") {
                    ...
                    error.data = { context: {} };
                    error.exceptionName = "modeem.exceptions.UserError";
                    throw error;
                }
            },
        });
    ...
    QUnit.test("Action Button clicked with failing action", async function (assert) {
        ...
        // fake error service so that the modeem qunit handlers don't think that they need to handle the error
