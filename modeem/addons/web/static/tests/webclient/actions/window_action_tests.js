...
import { onMounted } from "@modeem/owl";
let serverData;
let target;
...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("flags field of ir.actions.act_window is used", async function (assert) {
        // more info about flags field : https://github.com/modeem/modeem/commit/c9b133813b250e89f1f61816b0eabfb9bee2009d
        assert.expect(6);
    ...
    QUnit.test("debugManager is active for (legacy) views", async function (assert) {
        ...
        patchWithCleanup(modeem, { debug: "1" });
    ...
    QUnit.test("window action in target new fails (onchange)", async (assert) => {
        ...
        registry
            ...
            .add("modeem.exceptions.ValidationError", WarningDialogWait);

        const mockRPC = (route, args) => {
            if (args.method === "onchange" && args.model === "partner") {
                const error = new RPCError();
                error.exceptionName = "modeem.exceptions.ValidationError";
                error.code = 200;
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"zopenerp":"Modeem","action":"Partners"}');
        await click(target.querySelector(".o_data_row .o_data_cell"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Modeem","action":"First record"}');
        await click(target.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Modeem","action":"Second record"}');
    });
