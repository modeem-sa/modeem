...
from modeem import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID, Command
from modeem.exceptions import MissingError, AccessError
from modeem.osv import expression
from modeem.tools import is_html_empty
from modeem.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not modeembot
    ...
    def message_post(self, *,
        ...
        if 'channel_ids' in kwargs:
            raise ValueError(_("Posting a message with channels as listeners is not supported since Modeem 14.3+. Please update code accordingly."))
        ...
        if msg_values['author_id'] and msg_values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(msg_values['author_id']).active:  # we dont want to add modeembot/inactive as a follower
    ...
    def _notify_by_email_get_base_mail_values(self, message, additional_values=None):
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in Modeem)
