...
QUnit.test('display partner mention suggestions on typing "@"', async function (assert) {
    ...
    const resPartnerId1 = pyEnv['res.partner'].create({ email: "testpartner@modeem.com", name: "TestPartner" });
    const resPartnerId2 = pyEnv['res.partner'].create({ email: "testpartner2@modeem.com", name: "TestPartner2" });
...
QUnit.test('mention a partner', async function (assert) {
    ...
    const pyEnv = await startServer();
    const resPartnerId = pyEnv['res.partner'].create({ email: "testpartner@modeem.com", name: "TestPartner" });
    const mailChannelId1 = pyEnv['mail.channel'].create({
        channel_member_ids: [
            [0, 0, { partner_id: pyEnv.currentPartnerId }],
