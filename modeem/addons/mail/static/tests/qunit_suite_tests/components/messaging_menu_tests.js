...
QUnit.test('rendering with ModeemBot has a request (default)', async function (assert) {
    assert.expect(4);
    ...
    assert.ok(
        ...
        "should display a notification counter next to the messaging menu for ModeemBot request"
    );
    assert.strictEqual(
        document.querySelector('.o_MessagingMenu_counter').textContent,
        "1",
    ...
    assert.strictEqual(
        ...
        'ModeemBot has a request',
        "notification should display that ModeemBot has a request"
    );
});

QUnit.test('rendering without ModeemBot has a request (denied)', async function (assert) {
    assert.expect(2);
...
QUnit.test('rendering without ModeemBot has a request (accepted)', async function (assert) {
    assert.expect(2);
