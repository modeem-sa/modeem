...
QUnit.test('receive new chat message: out of modeem focus (notification, channel)', async function (assert) {
    assert.expect(4);
    ...
    const { env, openDiscuss } = await start({
        services: {
            presence: makeFakePresenceService({ isModeemFocused: () => false }),
        },
    });
    await openDiscuss();
    ...
    // simulate receiving a new message with modeem focused
    await afterNextRender(() => {
...
QUnit.test('receive new chat message: out of modeem focus (notification, chat)', async function (assert) {
    assert.expect(4);
...
QUnit.test('receive new chat messages: out of modeem focus (tab title)', async function (assert) {
    assert.expect(12);
    ...
    const mailChannel1 = pyEnv['mail.channel'].searchRead([['id', '=', mailChannelId1]])[0];
    // simulate receiving a new message in chat 1 with modeem focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel1, 'mail.channel/new_message', {
            'id': mailChannelId1,
            'message': {
    ...
    // simulate receiving a new message in chat 2 with modeem focused
    await afterNextRender(() => {
    ...
    // simulate receiving another new message in chat 2 with modeem focused
    await afterNextRender(() => {
        pyEnv['bus.bus']._sendone(mailChannel2, 'mail.channel/new_message', {
            'id': mailChannelId2,
            'message': {
...
QUnit.test("Message shows up even if channel data is incomplete", async function (assert) {
    ...
    const correspondentPartnerId = pyEnv["res.partner"].create({ name: "Albert", user_ids: [correspondentUserId] });
    ...
    await click(".o_DiscussSidebar_categoryChat .o_DiscussSidebarCategoryItem:contains(Albert)");
    assert.containsOnce(document.body, ".o_Message:contains(hello world)");
});
