...
registerModel({
    ...
    recordMethods: {
        ...
        async _handleNotificationChannelMessage({ id: channelId, message: messageData }) {
            ...
            // Chat from ModeemBot is considered disturbing and should only be
            // shown on the menu, but no notification and no thread open.
            const isChatWithModeemBot = (
                channel.correspondent &&
                channel.correspondent === this.messaging.partnerRoot
            );
            if (!isChatWithModeemBot) {
                const isModeemFocused = this.env.services['presence'].isModeemFocused();
                // Notify if out of focus
                if (!isModeemFocused && channel.thread.isChatChannel) {
