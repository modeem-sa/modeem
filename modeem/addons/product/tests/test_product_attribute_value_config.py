...
from modeem.tests import tagged, TransactionCase
from modeem.tools import mute_logger

from modeem.addons.base.tests.common import DISABLED_MAIL_CONTEXT
...
class TestProductAttributeValueConfig(TestProductAttributeValueCommon):
    ...
    def test_constraints(self):
        ...
        with mute_logger('modeem.sql_db'), self.assertRaises(IntegrityError, msg="can't have two values with the same name for the same attribute"):
            self.env['product.attribute.value'].create({
                'name': '32 GB',
                'attribute_id': self.ram_attribute.id,
            })
