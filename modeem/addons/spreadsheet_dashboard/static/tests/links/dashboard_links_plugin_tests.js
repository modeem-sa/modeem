...
QUnit.test("click a web link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Modeem](https://modeem.com)" } },
            },
        ],
    };
    const model = new Model(data, { mode: "dashboard", evalContext: { env } });
    selectCell(model, "A1");
    assert.verifySteps(["https://modeem.com"]);
});
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](modeem://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { mode: "dashboard", evalContext: { env } });
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: `[a view](modeem://view/${JSON.stringify(view)})` } },
            },
        ],
    };
    const model = new Model(data, { mode: "dashboard", evalContext: { env } });
