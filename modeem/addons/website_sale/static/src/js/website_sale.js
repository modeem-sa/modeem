modeem.define('website_sale.cart', function (require) {
'use strict';
...
modeem.define('website_sale.website_sale_offcanvas', function (require) {
'use strict';
...
modeem.define('website_sale.website_sale', function (require) {
'use strict';
        ...
        if (salePage.dataset.ecomZoomAuto) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomModeem = $image.data("zoomModeem");
                    if (zoomModeem) {
                        zoomModeem.hide();
                        $image.unbind();
                    }
        ...
        if (salePage.dataset.ecomZoomClick) {
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoommodeem-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.modeemEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.modeemEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
...
modeem.define('website_sale.price_range_option', function (require) {
'use strict';
