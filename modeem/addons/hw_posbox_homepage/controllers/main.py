...
from modeem import http, service
from modeem.http import Response
from modeem.modules.module import get_resource_path
from modeem.addons.hw_drivers.connection_manager import connection_manager
from modeem.addons.hw_drivers.main import iot_devices
from modeem.addons.hw_drivers.tools import helpers
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('modeem.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('modeem-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_modeem_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'modeem-remote-server.conf'
    ...
    def list_handlers(self):
        ...
        return handler_list_template.render({
            'title': "Modeem's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_modeem_server_url()
        })
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Modeem's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('modeem-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('modeem-enterprise-code.conf'),
        })

    @http.route('/save_credential', type='http', auth='none', cors='*', csrf=False)
    def save_credential(self, db_uuid, enterprise_code):
        helpers.write_file('modeem-db-uuid.conf', db_uuid)
        helpers.write_file('modeem-enterprise-code.conf', enterprise_code)
        helpers.modeem_restart(0)
    ...
    def clear_credential(self):
        helpers.unlink_file('modeem-db-uuid.conf')
        helpers.unlink_file('modeem-enterprise-code.conf')
        helpers.modeem_restart(0)
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to Modeem Server'
            }
        else:
            res_payload['server'] = {
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('modeem-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_modeem_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_modeem_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> Modeem server configuration',
            'breadcrumb': 'Configure Modeem Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_modeem_server_url() or 'Not configured yet',
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('modeem-six-payment-terminal.conf', terminal_id)
        service.server.restart()
        return 'http://' + helpers.get_ip() + ':8069'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('modeem-six-payment-terminal.conf')
        service.server.restart()
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"

    @http.route('/hw_proxy/upgrade', type='http', auth='none', )
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/modeem/", "--git-dir=/home/pi/modeem/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "Modeem's IoTBox - Software Upgrade",
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
    ...
    @http.route('/iot_restart_modeem_or_reboot', type='json', auth='none', cors='*', csrf=False)
    def iot_restart_modeem_or_reboot(self, action):
        """ Reboots the IoT Box / restarts Modeem on it depending on chosen 'action' argument"""
        try:
            if action == 'restart_modeem':
                helpers.modeem_restart(3)
            else:
