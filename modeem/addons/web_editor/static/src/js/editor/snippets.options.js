modeem.define('web_editor.snippets.options', function (require) {
'use strict';
...
const descendants = ModeemEditorLib.descendants;
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.modeemEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.modeemEditor.automaticStepActive('preview_option');
        }
...
registry.sizing = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ev.preventDefault();
            self.options.wysiwyg.modeemEditor.automaticStepUnactive('resizing');
            ...
            if (rowEl.classList.contains('o_grid_mode')) {
                self.options.wysiwyg.modeemEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                self.options.wysiwyg.modeemEditor.observerActive('displayBackgroundGrid');
            ...
            const bodyMouseUp = function () {
                ...
                self.options.wysiwyg.modeemEditor.automaticStepActive('resizing');
            };
...
￼    */
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Modeem).
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.modeemEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.modeemEditor.observerActive('_markColorLevel');
    },
});
