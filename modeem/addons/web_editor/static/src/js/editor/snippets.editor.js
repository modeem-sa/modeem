modeem.define('web_editor.snippet.editor', function (require) {
'use strict';
...
const {closestElement} = require('@web_editor/js/editor/modeem-editor/src/utils/utils');
...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        this.options.wysiwyg.modeemEditor.unbreakableStepUnactive();
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.modeemEditor.historyStep();
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.modeemEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.modeemEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.modeemEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart: function () {
        this.options.wysiwyg.modeemEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.modeemEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.modeemEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.modeemEditor.automaticStepActive();
        this.options.wysiwyg.modeemEditor.automaticStepSkipStack();
        this.options.wysiwyg.modeemEditor.unbreakableStepUnactive();
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.modeemEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.modeemEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.modeemEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.modeemEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.modeemEditor.historyCanRedo());
                };
                this.options.wysiwyg.modeemEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.modeemEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.modeemEditor.editable).trigger('content_changed');
                });
                this.options.wysiwyg.modeemEditor.addEventListener('historyRevert', _.debounce(() => {
        ...
        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                ...
                start: function () {
                    ...
                    const doc = self.options.wysiwyg.modeemEditor.document;
                    $(doc.body).addClass('oe_dropzone_active');

                    self.options.wysiwyg.modeemEditor.automaticStepUnactive();

                    self.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                    self.options.wysiwyg.modeemEditor.observerUnactive('dragAndDropCreateSnippet');
                ...
                stop: async function (ev, ui) {
                    ...
                    self.options.wysiwyg.modeemEditor.automaticStepUnactive();
                    self.options.wysiwyg.modeemEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe_snippet_body');
                    ...
                    self.options.wysiwyg.modeemEditor.observerActive('dragAndDropCreateSnippet');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.modeemEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.modeemEditor.historyStep();
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).modeemBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        if (!this._$toolbarContainer) {
            ...
            this.options.wysiwyg.modeemEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _addToolbarToOriginalPosition: function () {
        ...
        if (this.options.wysiwyg.modeemEditor.isMobile) {
            const editorEditable = this.options.wysiwyg.modeemEditor.editable;
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.modeemEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.modeemEditor.editable));
    },
    /**
