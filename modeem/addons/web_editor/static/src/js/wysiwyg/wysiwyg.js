modeem.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const ModeemEditorLib = require('@web_editor/js/editor/modeem-editor/src/ModeemEditor');
...
const ModeemEditor = ModeemEditorLib.ModeemEditor;
const getDeepRange = ModeemEditorLib.getDeepRange;
const getInSelection = ModeemEditorLib.getInSelection;
const isBlock = ModeemEditorLib.isBlock;
const rgbToHex = ModeemEditorLib.rgbToHex;
const preserveCursor = ModeemEditorLib.preserveCursor;
const closestElement = ModeemEditorLib.closestElement;
const setSelection = ModeemEditorLib.setSelection;
const endPos = ModeemEditorLib.endPos;
const hasValidSelection = ModeemEditorLib.hasValidSelection;
const parseHTML = ModeemEditorLib.parseHTML;
...
const Wysiwyg = Widget.extend({
    ...
    startEdition: async function () {
        ...
        this.modeemEditor = new ModeemEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.modeemEditor.addEventListener('contentChanged', function () {
        ...
        if (this.modeemEditor.document !== document) {
            this.modeemEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.modeemEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.modeemEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.modeemEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeModeemFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.modeemEditor.observerUnactive();
                const $field = $(this);
                ...
                self.modeemEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, function () {
            if (this.isContentEditable || (this.parentElement && this.parentElement.isContentEditable)) {
                ...
                const selection = self.modeemEditor.document.getSelection();
        ...
        if (options.snippets) {
            $(this.modeemEditor.document.body).addClass('editor_enable');
        ...
        $(this.modeemEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.modeemEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.modeemEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.modeemEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbar.el);

        this.modeemEditor.addEventListener('preObserverActive', () => {
            ...
            this._setONotEditable(this.modeemEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.modeemEditor.isMobile) {
                $(this.modeemEditor.editable).before(this.toolbar.$el);
            } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        this._getNewPtp = () => {
            ...
            return new PeerToPeer({
                ...
                onRequest: {
                    ...
                    get_missing_steps: (params) => this.modeemEditor.historyGetMissingSteps(params.requestPayload),
                    get_history_from_snapshot: () => this.modeemEditor.historyGetSnapshotSteps(),
                    get_collaborative_selection: () => this.modeemEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptp_remove':
                            this.modeemEditor.multiselectionRemove(notificationPayload);
                            break;
                        ...
                        case 'ptp_disconnect':
                            ...
                            this.modeemEditor.multiselectionRemove(fromClientId);
                            break;
                        ...
                        case 'oe_history_step':
                            ...
                            if (historySyncFinished) {
                                this.modeemEditor.onExternalHistorySteps([notificationPayload]);
                            } else {
                        ...
                        case 'oe_history_set_selection': {
                            ...
                            this.modeemEditor.onExternalMultiselectionUpdate(selection);
                            break;
                        }
                    }
                }
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                this.ptp && this.modeemEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
        };
    ...
    destroy: function () {
        ...
        if (this.modeemEditor) {
            this.modeemEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.modeemEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.modeemEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.modeemEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            for (const observer of this._oNotEditableObservers.values()) {
                observer.disconnect();
            }
            this.modeemEditor.destroy();
        }
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.modeemEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
        if (this.linkPopover) {
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper modeem-editor">');
            this.$root = $wrapper;
    ...
    getValue: function (options) {
        ...
        this.modeemEditor && this.modeemEditor.cleanForSave($editable[0]);
    ...
    historyReset: function () {
        this.modeemEditor.historyReset();
    },
    /**
     * Saves the content.
     *
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.modeemEditor.historySize().length) {
                resolve();
            } else {
    ...
    setValue: function (value) {
        this.modeemEditor.resetContent(value);
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.modeemEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.modeemEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (this.modeemEditor && !this.modeemEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.modeemEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    },
    getDeepRange() {
        return getDeepRange(this.modeemEditor.editable);
    },
    closestElement(...args) {
    ...
    async cleanForSave() {
        if (this.modeemEditor) {
            this.modeemEditor.cleanForSave();
    ...
    isSelectionInEditable: function () {
        return this.modeemEditor.isSelectionInEditable();
    },
    /**
     * Start or resume the Modeem field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeModeemFieldChanges: function () {
        ...
        if (this.modeemFieldObservers) {
            for (let observerData of this.modeemFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const modeemFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $modeemFields = this.$editable.find(modeemFieldSelector);
            this.modeemFieldObservers = [];

            $modeemFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.modeemEditor.filterMutationRecords(mutations);
                    if (!mutations.length) {
                        return;
                    }
                    let $node = $(field);
                    let $nodes = $modeemFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseModeemFieldObservers();
                ...
                this.modeemFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseModeemFieldObservers: function () {
        for (let observerData of this.modeemFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.modeemEditor.document.getSelection().getRangeAt(0).startContainer;
    ...
    toggleLinkTools(options = {}) {
        const linkEl = getInSelection(this.modeemEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                ...
                if (!this.linkTools || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkTools.$link[0])) {
                    const { link } = Link.getOrCreateLink({
                        containerNode: this.modeemEditor.editable,
                    ...
                    this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.modeemEditor.editable, linkToolsData, $btn, link );
                }
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.modeemEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.modeemEditor.document.addEventListener('click', _onClick, true);
            ...
            const historyStepIndex = this.modeemEditor.historySize() - 1;
            this.modeemEditor.historyPauseSteps();
            ...
            linkDialog.on('save', this, data => {
                ...
                this.modeemEditor.historyUnpauseSteps();
                this.modeemEditor.historyStep();
                link = linkWidget.$link[0];
                this.modeemEditor.setContenteditableLink(linkWidget.$link[0]);
            ...
            linkDialog.on('closed', this, function () {
                ...
                if (linkDialog.destroyAction !== 'save') {
                    ...
                    // occurs at the same time (but this case seems unlikely).
                    Promise.resolve().then(() => this.modeemEditor.historyRevertUntil(historyStepIndex));
                }
            });
        }
    },
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            ...
            this.modeemEditor.execCommand('unlink');
        }
    },
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.modeemEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.modeemEditor.document);

        const $editable = $(ModeemEditorLib.closestElement(range.startContainer, '.o_editable') || this.modeemEditor.editable);
    ...
    _onMediaDialogSave: function (params, element) {
        ...
        if (params.node) {
            ...
            this.modeemEditor.unbreakableStepUnactive();
            this.modeemEditor.historyStep();
        } else {
            return this.modeemEditor.execCommand('insert', element);
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.modeemEditor.document, selector);
    },
    /**
    ...
    _configureToolbar: function (options) {
        ...
        $toolbar.find('#image-crop').click(e => {
            ...
            new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(this.modeemEditor.document.body);
            this.modeemEditor.toolbarHide();
            $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.modeemEditor.toolbarShow());
        });
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.modeemEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.modeemEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.modeemEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.modeemEditor.document).on('mousedown', mousedown);
        });
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                this.modeemEditor.addDomListener(
                    scrollableContainer[0],
                    'scroll',
                    this.modeemEditor.updateToolbarPosition.bind(this.modeemEditor),
                );
            }
        }, 0);
    },
        ...
        for (const elem of $dropdownContent) {
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    this.modeemEditor.historyStash();
                    colorpicker = new ColorPaletteWidget(this, {
                        excluded: ['transparent_grayscale'],
                        $editable: $(this.modeemEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                    ...
                    colorpicker.on('custom_color_picked color_picked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.modeemEditor.historyResetLatestComputedSelection(true);
                        }
                        // Unstash the mutations now that the color is picked.
                        this.modeemEditor.historyUnstash();
                        ...
                        // hides it).
                        if (this.modeemEditor.deselectTable() && hasValidSelection(this.modeemEditor.editable)) {
                            this.modeemEditor.document.getSelection().collapseToStart();
                        }
                    ...
                    colorpicker.on('color_leave', null, ev => {
                        this.modeemEditor.historyRevertCurrentStep();
                    });
    ...
    _processAndApplyColor: function (eventName, color, previewMode) {
        ...
        const coloredElements = this.modeemEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        ...
        this.modeemEditor.updateColorpickerLabels({
    ...
    _updateEditorUI: function (e) {
        let selection = this.modeemEditor.document.getSelection();
        const anchorNode = selection.anchorNode;
        if (anchorNode && closestElement(anchorNode, '[data-oe-protected="true"]')) {
            return;
        }

        this.modeemEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.modeemEditor.document.defaultView;
        ...
        selection = this.modeemEditor.document.getSelection();
        ...
        if (isInMedia) {
            ...
            const selection = this.modeemEditor.document.getSelection();
            const range = this.modeemEditor.document.createRange();
        ...
        if (isInMedia) {
            ...
            this.tooltipTimeouts.push(setTimeout(() => {
                ...
                this.modeemEditor.observerUnactive();
                ...
                this.modeemEditor.observerActive();
            }, 400));
        }
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent ModeemEditor from controlling it as this is
        ...
        const commands = [
            {
                ...
                isDisabled: () => !this.modeemEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.modeemEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.modeemEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.modeemEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (res && res[0] && res[0].signature) {
                        this.modeemEditor.execCommand('insert', parseHTML(res[0].signature));
                    }
                },
            },
            {
                ...
                callback: () => this.modeemEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.modeemEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
                    const anchor = this.modeemEditor.document.getSelection().anchorNode;
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.modeemEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.modeemEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.modeemEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                isDisabled: () => {
                    if (!this.modeemEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
            ...
            },
            {
                ...
                fontawesome: 'fa-columns',
                callback: () => this.modeemEditor.execCommand('columnize', 0),
                isDisabled: () => {
                    if (!this.modeemEditor.isSelectionInBlockRoot()) {
                        return true;
                    }
    ...
    _saveElement: function ($el, context, withLang) {
        ...
        // remove ZeroWidthSpace from modeem field value
        // ZeroWidthSpace may be present from ModeemEditor edition process
    ...
    _onSelectionChange() {
        if (this.options.autohideToolbar) {
            ...
            if (isVisible && !this.modeemEditor.document.getSelection().isCollapsed) {
                this.linkPopover.hide();
            }
        }
    },
    ...
    _signalOnline: async function () {
        ...
        try {
            ...
            if (serverContent) {
                ...
                $dialogContent.append($(this.modeemEditor.editable).clone());
     ...
     *
     * See `_historyIds` in `historyReset` in ModeemEditor.
     *
     * @return {string} The database html content if the history is different.
     */
    async _ensureCommonHistory() {
        if (!this.ptp) return;
        const historyIds = this.modeemEditor.historyGetBranchIds();
        return this._rpc({
    ...
    resetEditor: async function (value, options) {
        ...
        this.modeemEditor.collaborationSetClientId(this._currentClientId);
        this.setValue(value);
        this.modeemEditor.historyReset();
        ...
        if (initialHistoryId) {
            this.modeemEditor.historySetInitialId(initialHistoryId);
        }
        await this._peerToPeerLoading;
    },
    /**
                    ...
                    for (const record of records) {
                        if (record.type === 'attributes' && record.attributeName === 'class') {
                            ...
                            if (!node.classList.contains('o_not_editable')) {
                                this.modeemEditor.observerUnactive('_setONotEditable');
                                node.removeAttribute('contenteditable');
                                this.modeemEditor.observerActive('_setONotEditable');
                                observer.disconnect();
    ...
    _attachHistoryIds(editable = this.modeemEditor.editable) {
        if (this.options.collaborative) {
            const historyIds = this.modeemEditor.historyGetBranchIds().join(',');
...
modeem.define('web_editor.widget', function (require) {
'use strict';
    return {
