modeem.define('wysiwyg.widgets.LinkTools', function (require) {
'use strict';
...
const LinkTools = Link.extend({
    ...
    async _addColorPicker(cssProperty) {
        ...
        const colorpicker = new ColorPaletteWidget(this, {
            ...
            $editable: $(this.options.wysiwyg.modeemEditor.editable),
        ...
        colorpicker.on('custom_color_picked color_picked color_hover color_leave', this, (ev) => {
            ...
            if (['custom_color_picked', 'color_picked'].includes(ev.name)) {
                ...
                this.options.wysiwyg.modeemEditor.execCommand('applyColor', '', 'color');
                this.options.wysiwyg.modeemEditor.execCommand('applyColor', '', 'backgroundColor');
            }
     ...
     */
    _addHintClasses () {
        this.options.wysiwyg.modeemEditor.observerUnactive("hint_classes");
        this.$link.addClass('oe_edited_link');
        this.$button.addClass('active');
        this.options.wysiwyg.modeemEditor.observerActive("hint_classes");
    },
    /**
    ...
    _removeHintClasses () {
        this.options.wysiwyg.modeemEditor.observerUnactive("hint_classes");
        $(this.options.wysiwyg.modeemEditor.document).find('.oe_edited_link').removeClass('oe_edited_link');
    ...
    __onURLInput() {
        this._super(...arguments);
        this.options.wysiwyg.modeemEditor.historyPauseSteps('_onURLInput');
        this._adaptPreview();
        this.options.wysiwyg.modeemEditor.historyUnpauseSteps('_onURLInput');
    },
});
