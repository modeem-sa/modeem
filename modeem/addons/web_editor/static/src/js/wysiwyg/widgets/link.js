modeem.define('wysiwyg.widgets.Link', function (require) {
'use strict';
...
const Link = Widget.extend({
    ...
    init: function (parent, options, editable, data, $button, link) {
        ...
        if (this.data.range) {
            this.$link = this.$link || $(ModeemEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
        ...
        if (!this.data.url) {
            const urls = this.data.content.match(ModeemEditorLib.URL_REGEX_WITH_INFOS);
            if (urls) {
