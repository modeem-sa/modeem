...
const LinkPopoverWidget = Widget.extend({
    ...
    start() {
        ...
        .on('show.bs.popover.link_popover', () => {
            this.options.wysiwyg.modeemEditor.observerUnactive('show.bs.popover');
            this._loadAsyncLinkPreview();
            popoverShown = true;
        })
        .on('inserted.bs.popover', () => {
            this.options.wysiwyg.modeemEditor.observerActive('show.bs.popover');
        })
        .on('hide.bs.popover.link_popover', () => {
            this.options.wysiwyg.modeemEditor.observerUnactive('hide.bs.popover');
            popoverShown = false;
        })
        .on('hidden.bs.popover.link_popover', () => {
            this.options.wysiwyg.modeemEditor.observerActive('hide.bs.popover');
        ...
        if (document !== this.options.wysiwyg.modeemEditor.document) {
            $(this.options.wysiwyg.modeemEditor.document).on('mouseup.link_popover', onClickDocument);
        }
    ...
    destroy() {
        ...
        $(this.options.wysiwyg.modeemEditor.document).off('.link_popover');
...
LinkPopoverWidget.createFor = async function (parent, targetEl, options) {
    ...
    if (wysiwyg) {
        wysiwyg.modeemEditor.observerUnactive('LinkPopoverWidget');
    }
    await popoverWidget.appendTo(targetEl)
    if (wysiwyg) {
        wysiwyg.modeemEditor.observerActive('LinkPopoverWidget');
    }
