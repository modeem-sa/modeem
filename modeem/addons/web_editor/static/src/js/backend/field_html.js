/** @modeem-module alias=web_editor.field.html */
'use strict';
...
import {
    ...
    rightPos,
    getRangePosition
} from '../editor/modeem-editor/src/utils/utils';
...
var FieldHtml = basic_fields.DebouncedField.extend(DynamicPlaceholderFieldMixin).extend(TranslatableFieldMixin, {
    ...
    openDynamicPlaceholder: async function (baseModel, chain = []) {
        ...
        const onFieldChanged = (ev) => {
            this.wysiwyg.modeemEditor.editable.focus();
    ...
    _onLoadWysiwyg: function () {
        ...
        if (this.nodeOptions.cssEdit && this.wysiwyg) {
            ...
            $button.css({
                ...
                position: 'absolute',
                top: '5px',
                [_t.database.parameters.direction === 'rtl' ? 'left' : 'right']: modeem.debug && this.nodeOptions.codeview ? '40px' : '5px',
            });
        }
        $container.append($button);
        if (modeem.debug && this.nodeOptions.codeview) {
