...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/modeem-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.modeemEditor) {
            const selection = this.wysiwyg.modeemEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        this.wysiwyg.modeemEditor.editable.addEventListener("input", () =>
    ...
    toggleCodeView() {
        ...
        this.wysiwyg.modeemEditor.observerUnactive('toggleCodeView');
        if (this.state.showCodeView) {
            this.wysiwyg.modeemEditor.toolbarHide();
            const value = this.wysiwyg.getValue();
            this.props.update(value);
        } else {
            this.wysiwyg.modeemEditor.observerActive('toggleCodeView');
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.modeemEditor.execCommand('insert', t);
        }
    }
    ...
    async commitChanges({ urgent } = {}) {
        if (this._isDirty() || urgent) {
            ...
            if (this.wysiwyg) {
                ...
                this.wysiwyg.modeemEditor.observerUnactive('commitChanges');
                await this.wysiwyg.saveModifiedImages();
                if (this.props.isInlineStyle) {
                    await this._toInline();
                }
                this.wysiwyg.modeemEditor.observerActive('commitChanges');
            }
    ...
    async _toInline() {
        ...
        const $modeemEditor = $editable.closest('.modeem-editor-editable');
        ...
        $modeemEditor.removeClass('modeem-editor-editable');
        $editable.html(html);

        await toInline($editable, this.cssRules, this.wysiwyg.$iframe);
        $modeemEditor.addClass('modeem-editor-editable');
...
HtmlField.extractProps = ({ attrs, field }) => {
    ...
    return {
        ...
        codeview: Boolean(modeem.debug && attrs.options.codeview),
        placeholder: attrs.placeholder,
