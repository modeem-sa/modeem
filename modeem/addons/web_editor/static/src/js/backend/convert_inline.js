/** @modeem-module alias=web_editor.convertInline */
'use strict';

import { isBlock, rgbToHex } from '../editor/modeem-editor/src/utils/utils';
...
async function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('modeem-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('modeem-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const fonts = modeem.__DEBUG__.services["wysiwyg.fonts"];
