modeem.define('web_editor.loader', function (require) {
'use strict';
...
async function getWysiwygClass({moduleName = 'web_editor.wysiwyg', additionnalAssets = []} = {}) {
    if (!(await modeem.ready(moduleName))) {
        await loadWysiwyg(additionnalAssets);
        await modeem.ready(moduleName);
    }
    return modeem.__DEBUG__.services[moduleName];
}
