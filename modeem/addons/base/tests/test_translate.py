...
from modeem.tools import sql
from modeem.tools.translate import quote, unquote, xml_translate, html_translate, TranslationImporter
from modeem.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('modeem.tests.stats')
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('modeem.addons.base.models.ir_module.Module._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Modeem'})
    #     country_1.with_context(lang='fr_FR').name = 'Modeem_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Modeem2'})
    #     with self.assertRaises(IntegrityError), mute_logger('modeem.sql_db'):
    #         country_2.name = 'Modeem'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('modeem.sql_db'):
    #         country_3 = Country.create({'name': 'Modeem'})
