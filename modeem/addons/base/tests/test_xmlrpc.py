...
from modeem.exceptions import AccessDenied, AccessError
from modeem.http import _request_stack

import modeem.tools
from modeem.tests import common
from modeem.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, modeem.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
