...
from modeem.tests.common import HttpCase, tagged
from modeem.tools import mute_logger, logging
...
class TestHttpCase(HttpCase):
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('modeem.tests.common.ChromeBrowser.take_screenshot', return_value=None):
    ...
    def test_console_log_object(self):
        logger = logging.getLogger('modeem')
        level = logger.level
