...
import modeem
from modeem.http import Controller, route, dispatch_rpc, request, Response
from modeem.fields import Date, Datetime, Command
from modeem.tools import lazy, ustr
from modeem.tools.misc import frozendict
...
# User code must use the exceptions defined in ``modeem.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, modeem.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, modeem.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, modeem.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, modeem.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, modeem.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(modeem.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class ModeemMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, modeem is sending binary as base64 string.
...
xmlrpc.client.Marshaller = ModeemMarshaller
