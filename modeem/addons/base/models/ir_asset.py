...
import modeem
import modeem.modules.module  # get_manifest, don't from-import it
from modeem import api, fields, models, tools
...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, addons, installed, css, js, asset_paths, seen):
        ...
        for addon in self._topological_sort(tuple(addons)):
            for command in modeem.modules.module.get_manifest(addon)['assets'].get(bundle, ()):
    ...
    def _topological_sort(self, addons_tuple):
        ...
        def mapper(addon):
            manif = modeem.modules.module.get_manifest(addon)
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(modeem.conf.server_wide_modules or []).union(self.env.context.get('install_module', []))
    ...
    def _get_paths(self, path_def, installed, extensions=None):
        ...
        addon = path_parts[0]
        addon_manifest = modeem.modules.module.get_manifest(addon)
