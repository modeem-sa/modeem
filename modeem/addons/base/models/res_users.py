...
from modeem import api, fields, models, tools, SUPERUSER_ID, _, Command
from modeem.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from modeem.exceptions import AccessDenied, AccessError, UserError, ValidationError
from modeem.http import request, DEFAULT_LANG
from modeem.osv import expression
from modeem.service.db import check_super
from modeem.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    ...
    def _unlink_except_superuser(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Modeem (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: modeem.exceptions.AccessDenied when old password is wrong
        :raise: modeem.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Modeem portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Modeem is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Modeem in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.modeem.com/documentation/16.0/administration/install/deploy.html#https for details.",
                    source
                )
