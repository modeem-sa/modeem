...
import modeem
from modeem import api, fields, models, modules, tools, _
from modeem.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from modeem.exceptions import AccessDenied, UserError
from modeem.osv import expression
from modeem.tools.parse_version import parse_version
from modeem.tools.misc import topological_sort
from modeem.tools.translate import TranslationImporter
from modeem.http import request
from modeem.modules import get_module_path, get_module_resource
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'Modeem Enterprise Edition License v1.0'),
        ('OPL-1', 'Modeem Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('Modeem Enterprise Module', default=False)
    ...
    def _button_immediate_function(self, function):
        ...
        try:
            ...
            raise UserError(_("Modeem is currently processing a scheduled action.\n"
    ...
    def button_uninstall(self):
        un_installable_modules = set(modeem.conf.server_wide_modules) & set(self.mapped('name'))
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = modeem.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/modeem/addons directory to the new "modeem" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'modeem' subdirectory ?
                ...
                _logger.info('Copy downloaded module `modeem` to `%s`', server_dir)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.modeem.com/apps')
