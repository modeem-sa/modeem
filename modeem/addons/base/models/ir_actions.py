...
import modeem
from modeem import api, fields, models, tools, _, Command
from modeem.exceptions import MissingError, ValidationError, AccessError
from modeem.tools import frozendict
from modeem.tools.safe_eval import safe_eval, test_python_expr
...
class IrActionsServer(models.Model):
    ...
    Since Modeem 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: Modeem Environment on which the action is triggered
#  - model: Modeem Model of the record on which the action is triggered; is a void recordset
#  - record: record on which the action is triggered; may be void
#  - records: recordset of all records on which the action is triggered in multi-mode; may be void
#  - time, datetime, dateutil, timezone: useful Python libraries
#  - float_compare: Modeem function to compare floats based on specific precisions
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': modeem.exceptions.Warning,
            'UserError': modeem.exceptions.UserError,
            # record
            'record': record,
            'records': records,
            # helpers
