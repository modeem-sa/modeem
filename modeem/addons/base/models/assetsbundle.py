...
from modeem import release, SUPERUSER_ID, _
from modeem.http import request
from modeem.modules.module import get_resource_path
from modeem.tools import (func, misc, transpile_javascript,
    is_modeem_module, SourceMapGenerator, profiler,
    apply_inheritance_specs)
from modeem.tools.misc import file_open, html_escape as escape
from modeem.tools.pycompat import to_text
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    modeem.define('{self.name}.bundle.xml', function(require){{
                        'use strict';
        ...
        * `t-extend` deprecated attribute, used by the JavaScript Qweb.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<modeem>') else f'<templates>{asset.content}</templates>'
        ...
        return """
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    var modeem = window.top.modeem;
                    if (!modeem || !modeem.define) {
                        useAlert();
                        return;
                    }
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of modeem.define
                        modeem.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of modeem.define
                    modeem.define('AssetsBundle.ErrorMessage', function (require) {
                        'use strict';
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Modeem you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def __init__(self, bundle, inline=None, url=None, filename=None):
        ...
        self.is_transpiled = is_modeem_module(super().content)
