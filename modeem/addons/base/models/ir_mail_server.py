...
from modeem.tools import ustr, pycompat, formataddr, email_normalize, encapsulate_email, email_domain_extract, email_domain_normalize
...
class IrMailServer(models.Model):
    ...
    from_filter = fields.Char(
        ...
        help='Define for which email address or domain this server can be used.\n'
             'e.g.: "notification@modeem.com" or "modeem.com"')
    ...
    def _get_test_email_addresses(self):
        ...
        if self.from_filter:
            if "@" in self.from_filter:
                ...
                return self.from_filter, "noreply@modeem.com"
            # All emails will be sent from any address in the same domain
            default_from = self.env["ir.config_parameter"].sudo().get_param("mail.default.from", "modeem")
            return f"{default_from}@{self.from_filter}", "noreply@modeem.com"
        ...
        return email_from, 'noreply@modeem.com'
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the modeem-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the modeem-bin argument "smtp_ssl_private_key"
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Modeem Server does not support SMTP-over-SSL. "
        ...
        # Store the "from_filter" of the mail server / modeem-bin argument to  know if we
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-modeem".
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        - Mail server to use to send the email (None if we use the modeem-bin arguments)
        ...
        # 5: SMTP config in modeem-bin arguments
        from_filter = self.env['ir.config_parameter'].sudo().get_param(
            'mail.default.from_filter', tools.config.get('from_filter'))
