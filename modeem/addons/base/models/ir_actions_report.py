...
from modeem import api, fields, models, tools, SUPERUSER_ID, _
from modeem.exceptions import UserError, AccessError
from modeem.tools.safe_eval import safe_eval, time
from modeem.tools.misc import find_in_path
from modeem.tools import check_barcode_encoding, config, is_html_empty, parse_version
from modeem.http import request
from modeem.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Modeem so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Modeem start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Modeem with at least two workers to print a pdf version of the reports.')
    ...
    def _merge_pdfs(self, streams):
        ...
        for stream in streams:
            try:
                ...
                raise UserError(_("Modeem is unable to merge the generated PDFs."))
