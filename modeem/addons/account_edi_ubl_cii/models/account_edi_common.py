...
from modeem import _, models, Command
from modeem.tools import float_repr
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_invoice(self, journal, filename, tree, existing_invoice=None):
        ...
        # This has to be done after the first import in order to let Modeem compute the taxes before overriding if needed.
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the modeem values
        ...
        It is not possible to do it in Modeem.
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    modeem_xmlid for modeem_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
