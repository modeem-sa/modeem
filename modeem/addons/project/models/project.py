...
from modeem import api, Command, fields, models, tools, SUPERUSER_ID, _, _lt
from modeem.addons.rating.models import rating_data
from modeem.addons.web_editor.controllers.main import handle_history_divergence
from modeem.exceptions import UserError, ValidationError, AccessError
...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Modeem-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Modeem-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Modeem-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
