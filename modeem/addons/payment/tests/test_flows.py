...
from modeem.addons.payment.controllers.portal import PaymentPortal
...
class TestFlows(PaymentHttpCommon):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger('modeem.http'):
            response = self._portal_transaction(**transaction_values)
        self.assertIn(
            "modeem.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.",
            response.text)
    ...
    def test_transaction_wrong_token(self):
        ...
        self.assertIn(
            "modeem.exceptions.ValidationError: The access token is invalid.",
            response.text)
    ...
    @mute_logger('modeem.addons.payment.models.payment_transaction')
