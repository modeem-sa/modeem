modeem.define('point_of_sale.Chrome', function(require) {
    'use strict';
    ...
    const { modeemExceptionTitleMap } = require("@web/core/errors/error_dialogs");
    ...
    class Chrome extends PosComponent {
        ...
        _errorHandler(error, errorToHandle) {
            if (errorToHandle instanceof RPCError) {
                ...
                if (modeemExceptionTitleMap.has(errorToHandle.exceptionName)) {
                    const title = modeemExceptionTitleMap.get(errorToHandle.exceptionName).toString();
