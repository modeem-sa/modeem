...
copy_raspios () {
    ...
    wget -c 'https://nightly.modeem.com/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.modeem.com/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v modeem
    mount -v "${PART_IOTBOX_ROOT}" modeem
    cp -v /home/pi/upgrade.sh modeem/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' modeem/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' modeem/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} modeem/home/pi/ \;
