...
function connect () {
	SERVER="${1}"
	CURRENT_SERVER_FILE=/home/pi/modeem-remote-server.conf
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_wifi "Restarting modeem"
		fi
		...
		/home/pi/modeem/addons/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
