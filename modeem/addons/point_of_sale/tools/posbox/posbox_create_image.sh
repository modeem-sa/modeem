...
REPO=https://github.com/modeem/modeem.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/modeem"
...
modeem/
modeem-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/modeem
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/modeem/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
