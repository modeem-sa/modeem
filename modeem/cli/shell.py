...
class Shell(Command):
    """Start modeem in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        modeem.cli.server.report_configuration()
        modeem.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = modeem.api.Environment(cr, uid, {})['res.users'].context_get()
                env = modeem.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
