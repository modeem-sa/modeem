...
__author__ = modeem.release.author
__version__ = modeem.release.version

# Also use the `modeem` logger for the main script.
_logger = logging.getLogger('modeem')
...
def report_configuration():
    ...
    """
    config = modeem.tools.config
    _logger.info("Modeem version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', modeem.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = modeem.tools.config
    if not modeem.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = modeem.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            modeem.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = modeem.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = modeem.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    modeem.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                modeem.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except modeem.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        modeem.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = modeem.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the modeem server (default command)"""
    def run(self, args):
        main(args)
