...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = modeem.tools.config.parser
        ...
        opt = modeem.tools.config.parse_config(args)

        dbname = modeem.tools.config['db_name']
        if not dbname:
        ...
        try:
            with modeem.sql_db.db_connect(dbname).cursor() as cursor:
                installed_modules = modeem.modules.neutralize.get_installed_modules(cursor)
                queries = modeem.modules.neutralize.get_neutralization_queries(installed_modules)
                if opt.to_stdout:
