    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('modeem.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'modeem.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Modeem models.

    Modeem models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~modeem.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~modeem.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~modeem.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Modeem sense)
    ...
    .. warning::
      ...
      :attr:`~modeem.models.Model._inherits`-ed models, the inherited field will
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = modeem.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], modeem.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], modeem.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from modeem.fields import Char, Integer
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(modeem.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(modeem.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~modeem.addons.base.models.res_company`).

        For :class:`~modeem.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from modeem.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~modeem.fields.Integer`,
          :class:`~modeem.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~modeem.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~modeem.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~modeem.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~modeem.fields.One2many` or
          :class:`~modeem.fields.Many2many` relational field is a list of
          :class:`~modeem.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~modeem.fields.Command.create`,
          :meth:`~modeem.fields.Command.update`,
          :meth:`~modeem.fields.Command.delete`,
          :meth:`~modeem.fields.Command.unlink`,
          :meth:`~modeem.fields.Command.link`,
          :meth:`~modeem.fields.Command.clear`, and
          :meth:`~modeem.fields.Command.set`.
        * For :class:`~modeem.fields.Date` and `~modeem.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`modeem.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`modeem.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def ensure_one(self):
        ...
        :raise modeem.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~modeem.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~modeem.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/modeem/master-nochange-fp/modeem/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`modeem.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Modeem models.

    Modeem models are created by inheriting from this class::
