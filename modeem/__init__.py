...
#----------------------------------------------------------
# modeem must be a namespace package for modeem.addons to become one too
...
import sys
assert sys.version_info > (3, 7), "Outdated python version detected, Modeem requires Python >= 3.7 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in modeem/debian/copyright)
        while 1:
            state = conn.poll()
...
from . import fields
from . import api
from modeem.tools.translate import _, _lt
