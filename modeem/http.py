...
r"""\
Modeem HTTP layer / WSGI application
...
:class:`~modeem.http.Controller`: class and its companion the
:func:`~modeem.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Modeem http request. The Modeem http request is
...
Modeem URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.modeem.com/documentation/16.0/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Modeem via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~modeem.service.db.list_dbs`:
    ...
    """
    try:
        dbs = modeem.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Modeem will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': modeem.service.common.dispatch,
        'db': modeem.service.db.dispatch,
        'object': modeem.service.model.dispatch,
    }
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    warnings.warn('modeem.http.send_file is deprecated, please use modeem.http.Stream instead.', DeprecationWarning, stacklevel=2)
    return _send_file(
        filepath_or_fp,
...
class Stream:
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`modeem.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~modeem.http.Controller` can use the :func:`~modeem.http.route`:
    ...
    :func:`~modeem.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(modeem.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['modeem', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~modeem.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['modeem', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Modeem addons.
        """
        # Controllers defined outside of modeem addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of modeem addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, login=None, password=None):
        ...
        with registry.cursor() as cr:
            env = modeem.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            ...
            request.env = modeem.api.Environment(request.env.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~modeem.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~modeem.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~modeem.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            self.env = modeem.api.Environment(cr, self.session.uid, self.session.context)
            threading.current_thread().uid = self.env.uid
            try:
                return service_model.retrying(self._serve_ir_http, self.env)
            except Exception as exc:
                if isinstance(exc, HTTPException) and exc.code is None:
                    raise  # bubble up to modeem.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~modeem.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Modeem Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in modeem.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + modeem.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = modeem.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if modeem.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
