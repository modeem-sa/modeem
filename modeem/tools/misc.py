...
import modeem
import modeem.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from modeem.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if modeem.tools.config['db_host']:
        env['PGHOST'] = modeem.tools.config['db_host']
    if modeem.tools.config['db_port']:
        env['PGPORT'] = str(modeem.tools.config['db_port'])
    if modeem.tools.config['db_user']:
        env['PGUSER'] = modeem.tools.config['db_user']
    if modeem.tools.config['db_password']:
        env['PGPASSWORD'] = modeem.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = modeem.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/modeem/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = modeem.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('modeem.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('modeem.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `modeem.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = modeem.fields.Datetime.from_string(value)
            value = modeem.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = modeem.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = modeem.fields.Datetime.from_string(value)
    else:
        timestamp = value
