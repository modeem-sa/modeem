...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except modeem.exceptions.UserError:
        raise
    except modeem.exceptions.RedirectWarning:
        raise
...
`modeem.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `modeem.tools.safe_eval`.
""")
    return d
