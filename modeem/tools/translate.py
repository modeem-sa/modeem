...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return modeem.sql_db.db_connect(db_name)
    ...
    def _get_lang(self, frame):
        ...
        if not lang:
            ...
            if not lang:
                ...
                if cr and uid:
                    env = modeem.api.Environment(cr, uid, {})
        ...
        try:
            ...
            if lang and lang != 'en_US':
                if not module:
                    ...
                    path_info = modeem.modules.get_resource_from_path(path)
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return Modeem translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Modeem translation entries """
    def __init__(self, source):
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/modeem/owl/blob/master/doc/reference/component.md#composition
...
class TranslationModuleReader:
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self.env = modeem.api.Environment(cr, modeem.SUPERUSER_ID, {})
        self._to_translate = []
        self._path_list = [(path, True) for path in modeem.addons.__path__]
    ...
    def __iter__(self):
        ...
        for module, source, name, res_id, ttype, comments, _record_id, value in self._to_translate:
            yield (module, ttype, name, res_id, source, encode(modeem.tools.ustr(value)), comments)
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'modeem.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in fnmatch.filter(files, '*_dashboard.json'):
                        self._babel_extract_terms(fname, path, root, 'modeem.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    Model = modeem.registry(cr.dbname)[field.model_name]
