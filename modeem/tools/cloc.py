...
import modeem
from modeem.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in modeem.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [modeem.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(modeem.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be modeem/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = modeem.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = modeem.SUPERUSER_ID
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Modeem cloc", lines="Line", other="Other", code="Code")
        s += hr
