...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.modeemrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Modeem data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'modeem_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "modeem.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="modeem.http:DEBUG", help='shortcut for --log-handler=modeem.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="modeem.sql_db:DEBUG", help='shortcut for --log-handler=modeem.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Modeem to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes modeem.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            modeem.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        modeem.netsvc.init_logger()
        self._warn_deprecated_options()
        modeem.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'modeem.conf')
        else:
            rcfilepath = os.path.expanduser('~/.modeemrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.modeemrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.modeemrc'.")
    ...
    def _is_addons_path(self, path):
        from modeem.modules.module import MANIFEST_NAMES
