...
class modeem_resolver(etree.Resolver):
    """Modeem specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None, prefix=None):
    ...
    :param modeem.api.Environment env: If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(modeem_resolver(env, prefix))
...
def load_xsd_files_from_url(env, url, file_name, force_reload=False,
    ...
    :param modeem.api.Environment env: environment of calling module
    ...
    :rtype: modeem.api.ir.attachment | bool
...
def validate_xml_from_attachment(env, xml_content, xsd_name, reload_files_function=None, prefix=None):
    ...
    :return: the result of the function :func:`modeem.tools.xml_utils._check_with_xsd`
    """
