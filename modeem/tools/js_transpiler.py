"""
This code is what let us use ES6-style modules in modeem.
Classic Modeem modules are composed of a top-level :samp:`modeem.define({name},{body_function})` call.
This processor will take files starting with an `@modeem-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** modeem-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual modeem one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom modeem modules.
    ...
    legacy_modeem_define = get_aliased_modeem_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_modeem_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_modeem_define:
        content += legacy_modeem_define
    return content
...
def url_to_module_path(url):
    """
    Modeem modules each have a name. (modeem.define("<the name>", async function (require) {...});
...
def wrap_with_modeem_define(module_path, content):
    """
    Wraps the current content (source code) with the modeem.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""modeem.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def is_modeem_module(content):
    """
    Detect if the file is a native modeem module.
    We look for a comment containing @modeem-module.

    :param content: source code
    :return: is this a modeem module that need transpilation ?
    """
    result = ODOO_MODULE_RE.match(content)
    return bool(result)


def get_aliased_modeem_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the modeem source base being:
    ...
    > modeem.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @modeem-module */
        // after
        /** @modeem-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @modeem-module */
        // after
        /** @modeem-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nmodeem.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\nmodeem.define(`%s`, async function(require) {
