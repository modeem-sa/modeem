...
from modeem import SUPERUSER_ID, api
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=modeem.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
                  relativedelta=relativedelta,
                  version=modeem.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('modeem.tools.convert.init').error(
        ...
        # invoke method
        return modeem.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
        ...
        if rec.get('groups'):
            ...
            for group in g_names:
                if group.startswith('-'):
                    ...
                    groups_value.append(modeem.Command.unlink(group_id))
                else:
                    group_id = self.id_get(group)
                    groups_value.append(modeem.Command.link(group_id))
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                group_id = self.id_get(group[1:])
                groups.append(modeem.Command.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(modeem.Command.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec, extra_vals=None):
        ...
        for field in rec.findall('./field'):
            ...
            if f_search:
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [modeem.Command.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler modeem.tools.convert:DEBUG" for complete traceback
    ...
    def __init__(self, cr, module, idref, mode, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.envs = [modeem.api.Environment(cr, SUPERUSER_ID, {})]
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <modeem> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['modeem', 'data', 'openerp']
