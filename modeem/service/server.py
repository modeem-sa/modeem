...
import modeem
from modeem.modules import get_modules
from modeem.modules.registry import Registry
from modeem.release import nt_service_name
from modeem.tools import config
from modeem.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'modeem.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by Modeem.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(modeem, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in modeem.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="modeem.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from modeem.addons.base.models.ir_cron import ir_cron
        conn = modeem.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = modeem.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(modeem.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="modeem.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="modeem.service.httpd")
        t.daemon = True
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(modeem, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = modeem.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('modeem: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(modeem.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = modeem.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from modeem.addons import base
    ...
    def start(self):
        ...
        dbconn = modeem.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(modeem.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            modeem.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if modeem.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from modeem.tests.common import ModeemSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = modeem.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = modeem.api.Environment(cr, modeem.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             modeem.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the modeem http server and cron processor.
    """
    global server
    ...
    if modeem.evented:
        server = GeventServer(modeem.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(modeem.http.root)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Modeem is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(modeem.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not modeem.evented:
        if inotify:
    ...
    if getattr(modeem, 'phoenix', False):
        _reexec()
