...
from modeem.tools.translate import _, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = modeem.registry(db).check_signaling()
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    result = retrying(partial(modeem.api.call_kw, recs, method, args, kw), env)
...
def execute(db, uid, obj, method, *args, **kw):
    with modeem.registry(db).cursor() as cr:
...
def retrying(func, env):
    ...
    :param modeem.api.Environment env: The environment where the registry
