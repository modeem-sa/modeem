...
import modeem
from modeem import SUPERUSER_ID
from modeem.exceptions import AccessDenied
import modeem.release
import modeem.sql_db
import modeem.tools
from modeem.sql_db import db_connect
from modeem.release import version_info
from modeem.tools import find_pg_tool, exec_pg_environ
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not modeem.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and modeem.tools.config.verify_admin_password(passwd):
        return True
    raise modeem.exceptions.AccessDenied()

# This should be moved to modeem.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = modeem.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            modeem.modules.db.initialize(cr)
            modeem.tools.config['load_language'] = lang
            cr.commit()

        registry = modeem.modules.registry.Registry.new(db_name, demo, None, update_module=True)
        ...
        with closing(registry.cursor()) as cr:
            ...
            if login:
                values['login'] = login
                emails = modeem.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = modeem.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = modeem.tools.config['db_template']
    ...
    try:
        db = modeem.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if modeem.tools.config['unaccent']:
                ...
                # But in the case of Modeem, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    modeem.sql_db.close_db(db_original_name)
    db = modeem.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = modeem.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = modeem.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = modeem.tools.config.filestore(db_original_name)
    to_fs = modeem.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    modeem.modules.registry.Registry.delete(db_name)
    modeem.sql_db.close_db(db_name)
    ...
    fs = modeem.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'modeem_dump': '1',
        'db_name': cr.dbname,
        'version': modeem.release.version,
        'version_info': modeem.release.version_info,
        'major_version': modeem.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = modeem.tools.config.filestore(db_name)
            ...
            if stream:
                modeem.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                modeem.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = modeem.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = modeem.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    modeem.modules.registry.Registry.delete(old_name)
    modeem.sql_db.close_db(old_name)
    ...
    old_fs = modeem.tools.config.filestore(old_name)
    new_fs = modeem.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    modeem.tools.config.set_admin_password(new_password)
    modeem.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        modeem.tools.config['update']['base'] = True
        modeem.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@modeem.tools.mute_logger('modeem.sql_db')
...
def list_dbs(force=False):
    if not modeem.tools.config['list_db'] and not force:
        raise modeem.exceptions.AccessDenied()

    if not modeem.tools.config['dbfilter'] and modeem.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Modeem will not
        ...
        res = sorted(db.strip() for db in modeem.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [modeem.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Modeem
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if modeem.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        modeem.sql_db.close_db(database_name)
...
def exp_list_lang():
    return modeem.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(modeem.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
