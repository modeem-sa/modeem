...
from .. import tools
from .common import TagsSelector, ModeemSuite
from .runner import ModeemTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'modeem.addons.{module}'))

    upgrade_spec = importlib.util.find_spec(f'modeem.upgrade.{module}')
    if upgrade_spec:
...
def make_suite(module_names, position='at_install'):
    ...
    return ModeemSuite(sorted(tests, key=lambda t: t.test_sequence))
