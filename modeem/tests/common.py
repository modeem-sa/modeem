# -*- coding: utf-8 -*-
"""
The module :mod:`modeem.tests.common` provides unittest test cases and a few
...
import modeem
from modeem import api
from modeem.models import BaseModel
from modeem.exceptions import AccessError
from modeem.modules.registry import Registry
from modeem.osv import expression
from modeem.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from modeem.service import security
from modeem.sql_db import BaseCursor, Cursor
from modeem.tools import float_compare, single_email_re, profiler, lower_logging
from modeem.tools.misc import find_in_path
from modeem.tools.safe_eval import safe_eval
...
# The modeem library is supposed already configured.
ADDONS_PATH = modeem.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = modeem.SUPERUSER_ID
...
def get_db_name():
    db = modeem.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Modeem module name.
    """
    def register(func):
        # register func by modeem module name
        if func.__module__.startswith('modeem.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most modeem operations;
    """
    if not login:
...
class ModeemSuite(BackportSuite):
    def _handleClassSetUp(self, test, result):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('modeem.addons.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Modeem-specific code. This class is abstract and
    ...
    longMessage = True      # more verbose error message by default: https://www.modeem.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~modeem.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        with patch('modeem.models.BaseModel.user_has_groups', user_has_groups):
            yield
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('modeem.sql_db.Cursor.execute', execute):
            with patch('modeem.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/modeem/addons/" in filename:
                        filename = filename.rsplit("/modeem/addons/", 1)[1]
                    if count > expected:
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/modeem/modeem/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (modeem_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def _gc_filestore(cls):
        ...
        with modeem.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, modeem.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.registry = modeem.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, modeem.SUPERUSER_ID, {})
...
class ChromeBrowser:
    ...
    def __init__(self, test_class):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_modeem')
        self.chrome_pid = None

        otc = modeem.tools.config
...
which leads to stray network requests and inconsistencies."""))
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Modeem widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{modeem.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('modeem.service.http.request.')]
        ...
        if request_threads:
            ...
            modeem.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        modeem.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        if getattr(self, 'session', None):
            modeem.http.root.session_store.delete(self.session)

        self.session = session = modeem.http.root.session_store.new()
        session.update(modeem.http.get_default_session(), db=get_db_name())
        session.context['lang'] = modeem.http.DEFAULT_LANG
        ...
        modeem.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{modeem.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "modeem.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "modeem.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('modeem.http.Request._get_profiler_context_manager', route_profiler))
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~modeem.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~modeem.fields.Many2many` fields, the field itself is a
    :class:`~modeem.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~modeem.fields.One2many` are reified as
    :class:`~modeem.tests.common.O2MProxy`.

    Because the :class:`~modeem.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~modeem.tests.common.O2MProxy.new` and
    :meth:`~modeem.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: modeem.models.Model
    ...
    :type view: int | str | modeem.model.Model
    ...
    def _process_fvg(self, model, fvg, level=2):
        ...
        * pre-processed onchanges list
        ...
        eval_context = {
            ...
            "datetime": datetime,
            "context_today": lambda: modeem.fields.Date.context_today(self._env.user),
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~modeem.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~modeem.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = modeem.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = modeem.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return modeem.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return modeem.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from modeem.tests.common have a test_tags
