...
import modeem
from modeem.tools import config, topological_sort, unique
from modeem.netsvc import init_logger
from modeem.tests import standalone_tests
import modeem.tests.loader

_logger = logging.getLogger('modeem.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with modeem.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Modeem modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Modeem modules")
...
def test_full(args):
    ...
    with modeem.registry(args.database).cursor() as cr:
        env = modeem.api.Environment(cr, modeem.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = modeem.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        modeem.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        modeem.tools.config['addons_path'] = ','.join([args.addons_path, modeem.tools.config['addons_path']])
        if args.data_dir:
            modeem.tools.config['data_dir'] = args.data_dir
        modeem.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('modeem.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('modeem.sql_db').setLevel(logging.CRITICAL)
